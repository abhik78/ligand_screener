...
def setup_screener(param_dir):
    ...
    settings.parameter_directory = param_dir
...
def parse_command_line_args():
    ...
    parser.add_argument('-p', '--param_dir', help = 'parameter files directory for screener setttings')
    ...
    parser.add_argument('-pl', '--pdb_ligand_dir', help='directory of all pdb ligands sdf')
...
class Processor:
    ...
    def fetch_overlay_mol(self, file_dir, id):
        for file in os.listdir(file_dir):
            if file.startswith('{}'.format(id)):
                overlay_mol = os.path.join(file_dir, file)
                return overlay_mol
    ...
    def select_largest_ligand(self, overlay_file):
        ...
        max_index, max_value = max(enumerate(atoms), key=operator.itemgetter(1))
        largest_ligand = overlay[max_index].identifier
        filepath = os.path.join(self.args.pdb_ligand_dir, '{}.sdf'.format(largest_ligand))
        return filepath
    ...
    def test_func(self, unp_id):
        ...
        query = [m for m in MoleculeReader(largest_ligand)]  # Read the query mol or overlay of mols
        print(query)
